...
The launch slice contains a forest-sector descriptor, water movement/material data, original mobile textures, a terrain heightfield, foliage LOD data, Aurora motion and palette data, GLES material metadata, the original/procedural forest audio bank, the authored `forest_prop_kit` source kit, and a separate Forest Warden boss source set. The prop kit contains original low-poly rocks, a fallen log, a ruin arch and wall, a camp fire ring/ember/wood pile, and a small shrine pedestal/standing stone. The boss source set contains a user-supplied mesh reduced to a 16,000-triangle near LOD and an 8,000-triangle mid LOD with transferred UVs and one diffuse material. Its boss, mobile, provenance, and player-preservation rules are recorded in `forest_warden_boss_asset.json`. Both source sets are distributed into the corresponding Gradle Play Asset Delivery modules under `assetpack_*/src/main/assets/launch_slice/`.

Stage 1 has a truthful **1 GiB source-budget envelope**. The currently authored payload is exactly **4,803,743 bytes (approximately 4.58 MiB)** of real content; the remaining Stage 1 budget is reserved for later authored packs and is never filled with padding. The separate full high-end Unreal plan remains deferred and is not presented as installed or downloadable.

The source OBJ/MTL prop kit and the derived Forest Warden OBJ/MTL set are production-ready source assets for the current pipeline, not cooked Unreal assets. The raw uploaded Forest Warden `model.obj` is intentionally excluded because it is approximately 2,000,000 triangles and 212 MB; no raw model.obj is shipped in the repository or Android payload. The preserved Aurora player contract remains separate from the boss asset. The following remain deferred until a real Unreal Engine 5.6 cook is run in a licensed Unreal build environment: imported `.uasset` static meshes and collision, `.umap` world sectors, cooked `.pak`/`.ucas`/`.utoc` files, production skeletal meshes and animation graphs, Niagara packages, platform shader libraries, cinematics, voice, and the future sand, snow, and dungeon expansions. Those packs remain deferred and must stay locked until their signed cooked payloads and source receipts exist.
