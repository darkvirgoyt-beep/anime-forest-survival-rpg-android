...
The Aethelgrad Android client presents only six server-approved profile avatar identifiers: `trailblazer`, `ember`, `verdant`, `tide`, `moon`, and `sunward`. The backend validates this same catalog and intentionally does **not** accept player-uploaded profile imagery.
