...
After a workflow run, download the `aethelgard-android-signing-certificate` artifact and register its **SHA1** value on the Android OAuth client in Google Cloud Console, together with package `com.darkvirgoyt.aethelgrad`. Keep an encrypted offline backup of the keystore outside GitHub; losing it means future production APKs must use a new Android OAuth SHA-1.
