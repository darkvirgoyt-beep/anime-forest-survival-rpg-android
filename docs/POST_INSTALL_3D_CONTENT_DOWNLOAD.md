...
Aethelgard now follows a **small bootstrap install plus required compiled-content download** model for production builds. The player installs the APK/AAB, signs in, chooses **Low Graphics** or **High Graphics**, downloads the selected complete world package, and only then reaches character setup and gameplay. The selected tier is saved on the device so later launches resume the same pack set instead of asking again.
...
| Resource choice | **Low Graphics** (about 4.2 GB) or **High Graphics** (about 6.6 GB) | The selected tier is stored and its Play Asset Delivery packs are requested |
...
The physical pack plan contains 18 production modules: one 358 MiB install-time core pack, one 350 MiB fast-follow forest launch pack, and 16 on-demand packs. The **Low Graphics** request uses the common world, character, dungeon, GLES, streaming, terrain, and animation modules for a planned 4,300 MiB (about 4.2 GB). The **High Graphics** request uses all 18 production modules for 6,750 MiB (displayed as 6.6 GB). These are pack budgets, not padding; the checked-in content remains intentionally compact until the authored production packs replace it.
