...
int main() {
    ...
    assert(restored.schemaVersion == 5 && restored.sourceSchemaVersion == 2 && restored.experience == 84);
    ...
    assert(forest::rpg::parseCloudState("{\"schemaVersion\":3,\"playerX\":0.1,\"playerY\":-0.1,\"health\":0.8,\"stamina\":0.7,\"hunger\":0.6,\"wood\":5,\"fiber\":6,\"stone\":7,\"experience\":120,\"level\":2,\"experienceToNext\":1500,\"totalExperience\":2120,\"day\":3,\"worldTime\":15,\"gatheringActions\":2,\"questStage\":1,\"emberKitCrafted\":1,\"wardenDefeated\":0}", restored));
    assert(restored.schemaVersion == 5 && restored.sourceSchemaVersion == 3);
    assert(restored.level == 2 && restored.totalExperience == 2120 && restored.emberKitCrafted);
    assert(restored.discoveredSectors == 1);

    assert(forest::rpg::parseCloudState("{\"schemaVersion\":4,\"playerX\":0.2,\"playerY\":-0.2,\"health\":0.9,\"stamina\":0.8,\"hunger\":0.7,\"wood\":8,\"fiber\":9,\"stone\":10,\"experience\":230,\"level\":3,\"experienceToNext\":1800,\"totalExperience\":3230,\"day\":5,\"worldTime\":25,\"gatheringActions\":3,\"questStage\":2,\"emberKitCrafted\":1,\"wardenDefeated\":0,\"emberlingTrust\":3,\"emberlingBonded\":1,\"emberlingStay\":1}", restored));
    ...
    assert(restored.schemaVersion == 5 && restored.sourceSchemaVersion == 1);
    ...
    const std::string migrated = forest::rpg::serializeCloudState(restored);
    assert(migrated.find("\"schemaVersion\":5") != std::string::npos);
    assert(migrated.find("\"discoveredSectors\":1") != std::string::npos);
