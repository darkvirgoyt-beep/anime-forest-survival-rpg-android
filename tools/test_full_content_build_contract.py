...
def main() -> None:
    ...
    full_budget_path = ROOT / "assets/full_content_budget.json"
    require(full_budget_path.is_file(), "deferred full-content budget manifest is missing")
    manifest = json.loads((ROOT / "app/src/main/assets/asset_manifest.json").read_text(encoding="utf-8"))

    high_tier = next((tier for tier in manifest["resourceTiers"] if tier.get("id") == "high"), None)
    require(high_tier is not None, "manifest must retain an explicit high tier")
    packs = high_tier["packs"]
    ...
    require("--manifest assets/full_content_budget.json" in build_script, "full-content build must use the deferred full budget manifest")
