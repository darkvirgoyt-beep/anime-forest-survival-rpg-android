...
def main() -> None:
    ...
    require("optionalStagePackNames" in plan, "runtime plan must identify non-blocking Stage 1 expansion packs")
    ...
    require("requestAvailableStageEnhancements" in catalog and "measuredPublishedPackNames" in catalog, "catalog must only auto-request real measured optional resources")
    require("requestDiscoveredSectorContent" in activity, "Android must trigger content requests from discovery state")
    require("requestAvailableStageExpansion" in activity and "LOCAL WORLD SIZE INCREASED" in activity, "Android must surface non-blocking measured expansion progress")
